#!/usr/bin/env python3
"""New ROVER v4: Final version with post-processing deduplication.

Improvements over v3:
- Post-processing to remove duplicate/overlapping lines
...
def is_substring_match(short: str, long: str, threshold: float = 0.9) -> bool:
    """Check if short is a substring (or near-substring) of long."""
    if len(short) >= len(long):
        return False
    # Check if short appears in long
    if short in long:
        return True
    # Check similarity of short to any substring of long
    for i in range(len(long) - len(short) + 1):
        substr = long[i : i + len(short)]
        if text_similarity(short, substr) >= threshold:
            return True
    return False
...
def extract_lines_easyocr(result: EngineResult) -> list[Paragraph]:
    """Extract lines from easyocr result."""
    if not result.items:
        return []
...
def deduplicate_lines(lines: list[str]) -> list[str]:
    ...
    Rules:
    1. If line A is a substring of line B, remove A
    2. If line A is very similar (>90%) to line B, keep the longer one
    """
    if not lines:
        return []
    ...
    for i, line_a in enumerate(lines):
        ...
        for j, line_b in enumerate(lines):
            ...
            if is_substring_match(line_a.strip(), line_b.strip()):
                should_skip_a = True
                break

            # Check if very similar
            sim = text_similarity(line_a, line_b)
            if sim >= 0.9:
                # Keep longer one
        ...
        if not should_skip_a:
            result.append(line_a)

    return result


def new_rover_v4(engine_results: dict[str, EngineResult]) -> tuple[str, dict[str, int]]:
    """New ROVER v4 with deduplication post-processing."""
    ...
    for para in yomitoku_paras:
        ...
        if overlapping:
            ...
            raw_lines.append(voted)
        else:
            raw_lines.append(para.text)
            contributions["yomitoku"] += 1

    # Post-processing: deduplicate
    deduped_lines = deduplicate_lines(raw_lines)

    return "\n".join(deduped_lines), contributions
...
def main():
    ...
    print("\nRunning NEW ROVER v4 (with dedup)...")
    merged_text, contributions = new_rover_v4(engine_results)
    ...
    # Compare with current ROVER
    current_rover_path = output_dir / "page_0024_rover_current.txt"
    if current_rover_path.exists():
        with open(current_rover_path, "r", encoding="utf-8") as f:
            current_text = f.read()
        current_content = "\n".join(line for line in current_text.split("\n") if not line.startswith("#"))
        current_sim = text_similarity(current_content.strip(), golden_content.strip())
        print(f"Current ROVER similarity to golden: {current_sim:.2%}")
        print(f"Improvement: {similarity - current_sim:+.2%}")

    output_file = output_dir / "page_0024_rover_new_v4.txt"
    with open(output_file, "w", encoding="utf-8") as f:
        f.write("# NEW ROVER v4 OCR output for page_0024.png\n")
        f.write("# 新ROVERアルゴリズム（段落ベース + 文字レベル投票 + 重複除去）\n")
