...
class TestEnginePriorityFallback:
    ...
    def test_no_fallback_key(self) -> None:
        ...
        assert "FALLBACK" not in ENGINE_PRIORITY, (
            "ENGINE_PRIORITY should not have 'FALLBACK' key. Rename to 'DEFAULT' for clarity."
        )
    ...
    def test_has_default_key(self) -> None:
        ...
        assert "DEFAULT" in ENGINE_PRIORITY, "ENGINE_PRIORITY should have 'DEFAULT' key instead of 'FALLBACK'."
    ...
    def test_default_engines_list(self) -> None:
        ...
        assert ENGINE_PRIORITY.get("DEFAULT") == expected, f"ENGINE_PRIORITY['DEFAULT'] should be {expected}"
...
class TestLegacyPatternRemoval:
    ...
    def test_no_legacy_comment(self) -> None:
        ...
        assert "Legacy patterns below for backward compatibility" not in source, (
            "Remove 'Legacy patterns below for backward compatibility' comment from parser.py"
        )
    ...
    def test_no_legacy_chapter_pattern_comment(self) -> None:
        ...
        assert "- legacy" not in source.lower(), "Remove '- legacy' annotation from pattern comments in parser.py"
    ...
    def test_no_redundant_case_insensitive_chapter_pattern(self) -> None:
        ...
        assert "[Cc][Hh][Aa][Pp][Tt][Ee][Rr]" not in source, (
            "Remove legacy case-insensitive Chapter pattern. The new pattern with re.IGNORECASE handles this case."
        )
